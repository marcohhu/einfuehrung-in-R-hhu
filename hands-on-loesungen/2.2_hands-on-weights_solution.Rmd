...
```{r}
#recode q23
df_post$wrt_akt[df_post$q23 == 1] <- 5
df_post$wrt_akt[df_post$q23 == 2] <- 4
df_post$wrt_akt[df_post$q23 == 3] <- 3
df_post$wrt_akt[df_post$q23 == 4] <- 2
df_post$wrt_akt[df_post$q23 == 5] <- 1
```
...
```{r}
#recode q25
df_post$wrt_reg[df_post$q25 == 1] <- 5
df_post$wrt_reg[df_post$q25 == 2] <- 4
df_post$wrt_reg[df_post$q25 == 3] <- 3
df_post$wrt_reg[df_post$q25 == 4] <- 2
df_post$wrt_reg[df_post$q25 == 5] <- 1
```

## Aufgabe 2: Gewichteten Datensatz erstellen
...
df.w <- svydesign(ids =~ 1, data = df_post, weights =~ w_ow)
```
...
```{r}
#mittelwert gewichtete Daten: wirtschaftliche Lage, aktuell
svymean(~ wrt_akt, df.w, na.rm = TRUE)

#mittelwert ungewichtete Daten
mean(df_post$wrt_akt, na.rm=TRUE)
```

```{r}
#mittelwert gewichtete Daten: Verantwortung wi
svymean(~ wrt_reg, df.w, na.rm = TRUE)

#mittelwert ungewichtete Daten
mean(df_post$wrt_reg, na.rm=TRUE)
```
